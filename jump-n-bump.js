...
function ViewModel() {
    ...
    this.levels = [{ name: 'caves' }, { name: 'cocaine' }, { name: 'green' }, { name: 'jump2' }, { name: 'kingofthehill' }, { name: 'mario' }, { name: 'sgeneral' }, { name: 'spring' }, { name: 'swamp' }, { name: 'thomas' }, { name: 'topsy' }, { name: 'waterfall' }];
    this.load_hosted_level = function (level) {
        self.loading_level(true);
        fetch('/levels/' + level.name + '/' + level.name + '.dat').then(function (response) {
            response.blob().then(function (data) {
                var file = new File([data], level.name + '.dat');
